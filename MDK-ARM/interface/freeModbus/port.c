...
// 声明保持寄存器缓冲区，用于存储十路保持寄存器的值  => [2] 表示电机设置圈数
// set_num = REG_HOLD_BUF[2] - 500
uint16_t REG_HOLD_BUF[REG_HOLD_SIZE] = {0, 0, 500, 0, 0, 0, 0, 0, 0, 0};

// 定义十路线圈的大小   =>  [2] 表示电机的启动停止
uint8_t REG_COILS_BUF[REG_COILS_SIZE] = {1, 1, 0, 1, 0, 0, 0, 0, 1, 1};

// 声明离散量缓冲区，并初始化，用于存储十路离散量的状态 => [3] 表示电机正反转
